...
def get_color_description(img_array, bins):
    ...
    img_array = img_array.astype(np.uint8)  # Ensure the image is in the correct format
    image = cv2.cvtColor(img_array, color)
    
    features = []

    # Print image shape and type for debugging
...
def histogram(image, mask, bins):
    '''
    calculate the color histogram of an image in specified channel using specified mask
    '''
